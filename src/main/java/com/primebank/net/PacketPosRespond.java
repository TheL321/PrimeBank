    ...
    public static class Handler implements IMessageHandler<PacketPosRespond, IMessage> {
        ...
        public IMessage onMessage(PacketPosRespond message, MessageContext ctx) {
            ...
            thread.addScheduledTask(() -> {
                ...
                if (message.companyId == null || !com.primebank.core.state.PrimeBankState.get().companies().isApproved(message.companyId)) {
                    p.sendMessage(new net.minecraft.util.text.TextComponentTranslation("primebank.pos.error.company_not_approved"));
                    return;
                }
                ...
                if (res.success) {
                    // English: Add gross sale to company's weekly sales accumulator (for valuation).
                    // Español: Agregar la venta bruta al acumulador semanal de la empresa (para valoración).
                    com.primebank.core.state.PrimeBankState.get().companies().addSales(message.companyId, message.cents);
                    ...
                    // Español: Aplicar cashback global si está configurado, pagado desde el central al comprador.
